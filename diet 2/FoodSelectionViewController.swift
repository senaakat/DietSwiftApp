...
class FoodSelectionViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, UISearchResultsUpdating {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let item = filteredItems[indexPath.row]
            ...
            cell.detailTextLabel?.text = "\(item.calories) kcal | C: \(item.carbs)g P: \(item.protein)g F: \(item.fat)g"
            return cell
    }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
       ...
       func updateSearchResults(for searchController: UISearchController) {
           ...
           filteredItems = items.filter { $0.name.lowercased().contains(query.lowercased()) }
           tableView.reloadData()
       }
}
