...
// 🔄 YENİ: TableView'da kaç section var
    ...
    func todaysMeals(for mealType: String) -> [FoodItem] {
        ...
        // 🔄 GÜNCELLENDİ: Sadece bugünün yemeklerinden toplam kalori hesapla
        func updateTotalCaloriesLabel() {
            let total = foodHistory
                .filter { Calendar.current.isDateInToday($0.date) }
                .reduce(0) { $0 + $1.calories }
            totalCaloriesLabel.text = "\(total) kcal"
        }
        func saveHistory() {
    ...
    func updateMacrosLabel() {
        let todayItems = foodHistory.filter { Calendar.current.isDateInToday($0.date) }

        let totalCarbs = todayItems.reduce(0) { $0 + $1.carbs }
        let totalProtein = todayItems.reduce(0) { $0 + $1.protein }
        let totalFat = todayItems.reduce(0) { $0 + $1.fat }

        carbsLabel.text = "Karbonhidrat: \(totalCarbs)g"
        proteinLabel.text = "Protein: \(totalProtein)g"
        fatLabel.text = "Yağ: \(totalFat)g"
    }
